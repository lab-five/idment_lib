use super::{mac_addr::MacAddress, rand_string::Rand, time::TimeFormatter};

pub struct Snowpunk {
    pub time_formatter: TimeFormatter,
    pub mac_formatter: MacAddress,
    pub rand_generator: Rand,
}

// 20位 时间码 + 12位 物理地址码 + 8位 随机码
impl Snowpunk {
    pub fn new() -> Self {
        Snowpunk {
            time_formatter: TimeFormatter {},
            mac_formatter: MacAddress {},
            rand_generator: Rand::new(),
        }
    }
    ...
    pub async fn build(&self) -> String {
        let snowpunk = format!(
            "{}{}{}",
            self.time_formatter.get_time().await,
            self.mac_formatter.get_mac().await,
            self.rand_generator.get_rand(8).await
        );
        snowpunk
    }
}
